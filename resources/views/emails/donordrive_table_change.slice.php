@extends('layouts.email')

@section('content')
Hello,
<br />
<p>We noticed a change in team {{ $teamname }} to the <b>{{ $database }}</b> DonorDrive database for the table <b>{{ $table }}</b>.</p>
@if(count($added) > 0)
    <p>The following columns were added in DonorDrive:</p>
    <ul>
        @foreach ($added as $column)
            <li>{{ $column }}</li>    
        @endforeach
    </ul>
@endif
@if(count($removed) > 0)
    <p>The following columns were removed in DonorDrive:</p>
    <ul>
        @foreach ($removed as $column)
            <li>{{ $column }}</li>   
        @endforeach
    </ul>
@endif
<p>We automatically rebuilt and re-synced this table, and all is well. You may want to check the conversion programming to make sure there are no additional actions needed.</p>
@if(count($conversions) > 0)
    <p>Blueprint indicates that the following fields are used in conversion:</p>
    <ul>
        @foreach ($conversions as $column)
            <li>{{ $column }}</li>    
        @endforeach
    </ul>
@endif
@endsection